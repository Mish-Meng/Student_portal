...
// Get teacher's assigned class grades for filtering
...
// Get teacher's assigned classes (for display)
$teacher_classes_display_query = mysqli_query($conn, "SELECT grade, subjects FROM classes WHERE teacher LIKE '%$teacher_name%' ORDER BY grade ASC");
$teacher_classes_display = [];
while ($row = mysqli_fetch_assoc($teacher_classes_display_query)) {
    $teacher_classes_display[] = $row;
}
...
// Fetch students (only teacher's students)
$students_query = mysqli_query($conn, "SELECT id, fullname, adm_no, class FROM students WHERE 1=1 $class_filter ORDER BY class, fullname ASC");
...
<div class="main">
  <div class="header">
    <div>
      ...
      <p>Record student attendance for your assigned classes</p>
      <?php if(!empty($teacher_classes_display)): ?>
        <div style="margin-top: 12px; display: flex; flex-wrap: wrap; gap: 8px;">
          <?php foreach($teacher_classes_display as $class): ?>
            <span style="background: rgba(255,114,0,0.2); border: 1px solid var(--accent); border-radius: 6px; padding: 4px 10px; font-size: 12px; color: var(--accent);" title="<?= h($class['subjects']) ?>">
